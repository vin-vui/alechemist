<div>
    ...
    <div class="flex flex-col gap-4 px-4 sm:px-6 lg:px-8 pt-8 pb-12">
        @foreach ($brewings as $brewing)
            <a href="{{ route('preparation', $brewing) }}">
                <div class="flex bg-gray-50 shadow-lg">
                    ...
                    {{-- <div class="flex justify-between pl-24 pr-4 pb-2">
                        <div class="text-xs">Create at {{ $brewing->created_at->format('Y-m-d') }}</div>
                        <div class="text-xs">Modify at {{ $brewing->updated_at->format('Y-m-d') }}</div>
                    </div> --}}
                </div>
            </a>
        @endforeach
