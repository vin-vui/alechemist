<div class="pb-20">
        ...
        <div class="max-w-full flex gap-4">
            ...
            <div class="flex flex-col truncate flex-1 mr-4 py-2">
                ...
                <div class="w-max grid grid-cols-2 gap-x-2 gap-y-1 mt-2">
                    <div class="text-sm font-semibold">{{ __('Alcohol') }}</div>
                    <div class="info-label-yellow text-xs text-center">{{ $recipe->alcohol }} %</div>
                    <div class="text-sm font-semibold">{{ __('Created at') }}</div>
                    <div class="info-label text-xs text-center">{{ $recipe->created_at->format('d/m/Y') }}</div>
                    <div class="text-sm font-semibold">{{ __('Updated at') }}</div>
    ...
    <div class="flex w-full items-center bg-gray-50 mb-8 py-4 pr-2">
        ...
        <div class="grid grid-cols-2 text-sm gap-4">
            ...
            <div class="grid grid-cols-2 gap-x-2 gap-y-1">
                <div class="font-semibold truncate">{{ __('Before Boil Density') }}</div>
                @if ($isOpen)
                ...
                @endif
                <div class="font-semibold truncate">{{ __('Initial Density') }}</div>
                <div class="info-label text-center text-xs">{{ $recipe->initial_density }}</div>
                <div class="font-semibold truncate">{{ __('Final Density') }}</div>
                <div class="info-label text-center text-xs">{{ $recipe->final_density }}</div>
                <div class="font-semibold truncate">{{ __('Carbonation') }}</div>
    ...
    <div class="sticky top-24 px-4 pb-4">
        <a href="{{ route('brewing.index', $recipe) }}" class="btn-yellow">
            ...
            <div>{{ Str::plural(__('Brewing'), $this->inProgressCount) }} {{ __('in progress') }}</div>
    ...
    <div class="">
        ...
        <div>
            <div class="flex items-center gap-x-2 p-2">
                ...
                <span>{{ __('Preparation') }}</span>
            </div>
            <div class="pb-4">
                <table class="w-full text-sm text-gray-500">
                    <thead class="text-xs text-gray-800 uppercase bg-old-gold">
                        <tr>
                            <th scope="col" class="text-left px-4 py-3">{{ __('Name') }}</th>
                            <th scope="col" class="text-right px-4 py-3">{{ __('Quantity') }}</th>
                        </tr>
                    </thead>
                    <tbody>
                        @foreach ($recipe->steps->where('type', 'Preparation')->where('unit', '<>', 'l') as $step)
        ...
        <div>
            ...
            <div class="pb-4">
                <table class="w-full text-sm text-gray-500">
                    <thead class="text-xs text-gray-800 uppercase bg-old-gold">
                        <tr>
                            <th scope="col" class="text-left px-2 py-3">{{ __('Name') }}</th>
                            <th scope="col" class="px-2 py-3">{{ __('Temp.') }}</th>
                            <th scope="col" class="text-right px-2 py-3">{{ __('Time') }}</th>
                        </tr>
                    </thead>
                    <tbody>
                        @foreach ($recipe->steps->where('type', 'Mash') as $step)
        ...
        <div>
            ...
            <div class="pb-4">
                <table class="w-full text-sm text-gray-500">
                    <thead class="text-xs text-gray-800 uppercase bg-old-gold">
                        <tr>
                            <th scope="col" class="text-left px-2 py-3">{{ __('Add') }}</th>
                            <th scope="col" class="text-left px-2 py-3">{{ __('Qty.') }}</th>
                            <th scope="col" class="text-right px-2 py-3">{{ __('Time') }}</th>
                        </tr>
                    </thead>
                    <tbody>
                        ...
                        <tr class="text-center bg-white border-b">
                            <td class="text-left px-2 py-3">{{ $step->field }} <span class="text-gray-300 text-xs">({{ __($step->type) }})</span></td>
                ...
                <table class="w-full text-sm text-gray-500">
                    ...
                    <tbody>
                        ...
                        <tr class="text-center bg-white border-b">
                            ...
                            <td class="text-left px-2 py-3">{{ $step->quantity }} {{ $step->unit }}</td>
                            <td class="text-right px-2 py-3">{{ $step->time / 1440 }} {{ __('days') }}</td>
                        </tr>
                        @endforeach
                    </tbody>
                </table>
        ...
        <div>
            ...
            <div class="pb-4">
                <table class="w-full text-sm text-gray-500">
                    <thead class="text-xs text-gray-800 uppercase bg-old-gold">
                        <tr>
                            <th scope="col" class="text-left px-2 py-3">{{ __('Step') }}</th>
                            <th scope="col" class="text-left px-2 py-3">{{ __('Temp.') }}</th>
                            <th scope="col" class="text-right px-2 py-3">{{ __('Time') }}</th>
                        </tr>
                    </thead>
                    <tbody>
