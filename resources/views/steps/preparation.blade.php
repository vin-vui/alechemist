                    ...
                    <div class="rounded-tl-md rounded-tr-md relative flex cursor-pointer p-4 focus:outline-none">
                        <div class="ml-3 flex gap-x-1 justify-between">
                            <div class="block text-sm font-medium">
                                @if($step->unit == 'l')
                                    Heat <span class="info-label-inprogress">{{ $step->quantity }} {{ $step->unit }}</span> at <span class="info-label-inprogress">{{ $step->field }}</span>
                                @else
                                    @if ($step->quantity >= 1000)
                                        <span class="info-label-inprogress">{{ $step->quantity / 1000 }} kg</span> {{ $step->field }}
                                    @else
                                        <span class="info-label-inprogress">{{ $step->quantity }} {{ $step->unit }}</span> {{ $step->field }}
                                    @endif
                                @endif
                            </div>
                        </div>
